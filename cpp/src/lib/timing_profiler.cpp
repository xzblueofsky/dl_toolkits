...
void timing_profiler::update(string& name)
{
  ...
  if(it != history.end()) {
	  history[name] = IIR_rate*time_pieces[name] + (1.0f-IIR_rate)*history[name];
  }
  else {
...
char* timing_profiler::getTimeProfileString(void)
{
  ...
  memset(profile_string, 0, 1024);
  for(it=time_pieces.begin(); it!=time_pieces.end(); it++)
  {
    sprintf(profile_string, "%s[%s]:%3.1f, ", profile_string, it->first.c_str(), it->second);
  }
